...
//-- MARCH 2017: added personal welcome..
//-- Jason Snow
//-----------------------------------------------------------------
...
*/
 ...
 //-----
 //----- NEW OTTO WELCOME  
 // display OTTO on LED MATRIX on start-up
 for (int i=0;i<8;i++){
...
void loop() {
  ...
  if (buttonPushed){  
    ...
    if      ( buttonAPushed && !buttonBPushed){ MODE=1; Otto.sing(S_mode1);}
    else if (!buttonAPushed && buttonBPushed) { MODE=2; Otto.sing(S_mode2);}
    else if ( buttonAPushed && buttonBPushed) { MODE=3; Otto.sing(S_mode3);} //else
